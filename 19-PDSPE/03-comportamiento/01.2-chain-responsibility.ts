...
class Supervisor extends BaseApprover {
	...
	override approveRequest(amount: number): void {
		if (amount <= 1000) console.log("Solicitud aprobada por el supervisor");
		else {
			console.log("Solicitud enviada a otra persona");
			super.next(amount);
		}
	}
}
...
class Manager extends BaseApprover {
	...
	override approveRequest(amount: number): void {
		if (amount <= 5000) console.log("Solicitud aprobada por el manager");
		else {
			console.log("Solicitud enviada a otra persona");
			super.next(amount);
		}
...
class Director extends BaseApprover {
	...
	override approveRequest(_amount: number): void {
		console.log("Solicitud aprobada por el Director");
	}
}
